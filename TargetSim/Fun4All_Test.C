...
int Fun4All_Test(const int nEvents = 1)
{
	...
	g4Reco->SetWorldMaterial("G4_Galactic");//G4_Galactic, G4_AIR
	// Geant4 Physics list to use
	g4Reco->SetPhysicsList("FTFP_BERT");

	//PHG4CylinderSubsystem *blackhole = new PHG4CylinderSubsystem("BH", 1);
	//blackhole->set_double_param("radius", 25); // add 10 cm
	//blackhole->set_int_param("lengthviarapidity",0);
	//blackhole->set_double_param("length",1); // make it cover the world in length
	//blackhole->set_double_param("rot_x", -90.);
	//blackhole->BlackHole();
	...
	PHG4CylinderSubsystem *dummy = new PHG4CylinderSubsystem("DUMMY", 1);
	dummy->set_string_param("material", "G4_AIR");          // material of target
	dummy->set_double_param("radius", 22); // add 10 cm
	dummy->set_int_param("lengthviarapidity",0);
	dummy->set_double_param("length",1); // make it cover the world in length
	dummy->set_double_param("rot_x", -90.);
	dummy->set_double_param("thickness",0.1); // it needs some thickness
	dummy->SetActive(); // always see what leaks out
	g4Reco->registerSubsystem(dummy);
